  ...
  try {
    ...
    if (user.credits < 25) {
      return res
        .status(400)
        .json({
          message: "you do not have enough credits to update the website",
        });
    }
...
export const deploy = async (req, res) => {
  try {
    ...
    // Mark as deployed and set deploy URL
    ...
    return res.status(500).json({
      message: `Deploy website error: ${error.message || error}`,
    });
  }
};
...
export const getBySlug = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      console.log(error)
      message: `Get By Slug error: ${error.message || error}`,
    });
  }
};
